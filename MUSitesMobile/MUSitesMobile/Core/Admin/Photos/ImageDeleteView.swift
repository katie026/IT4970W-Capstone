...
struct ImageDeleteView: View {
    ...
    private func loadImages() {
        if category == "Posters" {
            ...
            // For other categories, use the basePath
            StorageManager.shared.listImages(basePath: "\(basePath)/\(siteName)/", category: category) { result in
                switch result {
    ...
    private func deleteImage(_ imageName: String) {
        if category == "Posters" {
            ...
            // Use the basePath for deletion as well
            StorageManager.shared.deleteImage(basePath: "\(basePath)/\(siteName)/\(category)", imageName: imageName) { result in
                switch result {
                case .success():
